...
session_start();
$count = 0;
$myarr = ["phone","laptop","smart watch","TV","washing machine ", "Air Conditioner","footwear","Clothing","Grooming",
"Clothing","footwear","Grooming","Home Decor","Sports","toys"];
  ...
  if(isset($_COOKIE['voice'])){
    ...
    foreach($arr as $v){
      for ($i=0; $i < count($myarr); $i++) { 
        if(strtolower($v) == strtolower($myarr[$i])){
          $mytype12 = $myarr[$i];
        }
      }
    }
      for ($i=0; $i < count($myarr); $i++) { 
        if(strtolower($str) == strtolower($myarr[$i])){
          $mytype1 = $myarr[$i];
        }
      }
	...
	unset($_COOKIE['voice']);
  if(isset($mytype12)){
    $query="select * from product where name like '%$mytype%' OR category_id = '$mytype12'";
  }

  else{
  $query="select * from product where name like '%$mytype%' OR category_id = '$mytype1'";
}
}
